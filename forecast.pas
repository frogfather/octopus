...
var
  main, clouds, wind, weather : TJSONObject;
...
begin
  ...
  weatherArray:=forecastItem.Arrays['weather'];
  //Weather is an array of objects. For the moment we'll just get the first
  if (weatherArray.Count > 0) then
    begin
    weather:=TJSONObject(weatherArray[0]);
    FWeatherSummary:=weather.Get('main');
    FWeatherDescription:=weather.get('description');
    end;
